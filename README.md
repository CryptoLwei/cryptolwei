<div align="center">

<img src="https://avatars.githubusercontent.com/u/195447843" alt="CryptoLwei Logo" width="120" />

# 👋 Hey, I'm Augusto — aka CryptoLwei

**Crypto educator,trader and analytic turned on-chain builder.**

I started by trading and teaching people how crypto works.  
Then I started building it myself.

[![Website](https://img.shields.io/badge/cryptolwei.com-000000?style=flat&logo=google-chrome&logoColor=white)](https://cryptolwei.com)
[![X](https://img.shields.io/badge/@cryptolwei-000000?style=flat&logo=x&logoColor=white)](https://x.com/cryptolwei)
[![YouTube](https://img.shields.io/badge/CryptoLwei-FF0000?style=flat&logo=youtube&logoColor=white)](https://youtube.com/CryptoLwei)
[![Bluesky](https://img.shields.io/badge/Bluesky-0285FF?style=flat&logo=bluesky&logoColor=white)](https://bsky.app/profile/cryptolwei.bsky.social)

</div>

---

## 🧭 The Journey
2024 → Built cryptolwei.com — 20-page educational crypto site from scratch
HTML · Tailwind CSS · Vanilla JS

2024 → Expanded to cryptolwei.com v2 — evolved brand & portfolio site

2024 → Deployed first Ethereum smart contracts on HackQuest
ERC-20 token · Breedable ERC-721 NFTs · Decentralized crowdfunding

2025 → Built CryptoLwei DeFi PoC
Gamified NFT learning platform on Algorand · Python

text

---

## 🗂️ Repos

### 🌐 Web
| Repo | Description | Stack |
|------|-------------|-------|
| [cryptolwei-site-v1](https://github.com/cryptolwei/cryptolwei-site-v1) | Original educational site — the origin story | HTML · Tailwind · JS |
| [cryptolwei-site-v2](https://github.com/cryptolwei/cryptolwei-site-v2) | Current brand & portfolio site | HTML · CSS · JS |

### ⛓️ Ethereum (Solidity)
| Repo | Description | Standard |
|------|-------------|---------|
| [cryptolwei-token](https://github.com/cryptolwei/cryptolwei-token) | Custom token with owner-controlled minting | ERC-20 |
| [evolving-kitties-eth](https://github.com/cryptolwei/evolving-kitties-eth) | Breedable NFTs with on-chain genetics & lineage | ERC-721 |
| [cryptolwei-crowdfund](https://github.com/cryptolwei/cryptolwei-crowdfund) | Decentralized crowdfunding — pledge, claim, refund | ERC-20 |

### 🔷 Algorand
| Repo | Description | Stack |
|------|-------------|-------|
| [CryptoLwei-DeFi-PoC](https://github.com/cryptolwei/CryptoLwei-DeFi-PoC) | Gamified NFT learning platform with on-chain achievements | Python · Algorand |
| [Algorand-Web3-Masterclasses-POC](https://github.com/cryptolwei/Algorand-Web3-Masterclasses-POC) | Algorand Web3 masterclass PoC | Algorand |

---

## 🛠️ Tech
Blockchain Solidity · Ethereum · Algorand · AlgoKit · OpenZeppelin
Frontend HTML5 · Tailwind CSS · Python · TypeScript · Vanilla JS
Education HackQuest Web3 Certification

text

---

## 👥 Behind CryptoLwei

| | |
|---|---|
| **Augusto Faustino** | Founder · CEO & CTO |
| **Suphee Yangnok** | CCO & Research |

---

<div align="center">
  <i>Building in public. Learning on-chain.</i><br/><br/>
  📧 cryptolwei@gmail.com
</div>

